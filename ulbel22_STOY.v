...
// August 2015
//
// S-TOY2 consists of 4 modules: 
//      cpu + memory + stdin + stdout
//
// S-TOY2: 4 new instructions: Push, Pop, Call, Ret
//
...
`define PRINT_MEM_LOCATIONS 'hff  
      ...
      while(1)
        begin
          ...
          case (IR[15:12])
           ...
           'hA: begin // R[d] <- mem[R[t]]
                  @(posedge clk) enter_new_state(`LDI1); // load indirect
                  #`MAX_DELAY;
           ...
           'hB: begin // mem[R[t]] <- R[d]
                  @(posedge clk) enter_new_state(`STI1); // store indirect
                  #`MAX_DELAY;
                  MA  <= @(posedge clk) R[IR[3:0]];  
                  MDO <= @(posedge clk) R[IR[11:8]];
           ...
           'hE: if (IR[11:8] != 0)  
                    begin   // PC <- R[d]
                        @(posedge clk) enter_new_state(`JR); // jump register
                        #`MAX_DELAY;
           ...
           'hF: if (IR[11:8] != 0)
                   begin // R[d] <- PC, PC <- IR[7:0]
                        @(posedge clk) enter_new_state(`JL); // jump and link
                        #`MAX_DELAY;
...
////////////////////////////////////////////////////
module memory(clk, addr, din, dout, write);
  input clk;
  input [7:0]  addr;
  input  [15:0] din;
  output [15:0] dout;
...
////////////////////////////////////////////////////
module  stoy2(clk, ext_din, ext_write, ext_dout);
  input clk;
  input [15:0] ext_din;
  input ext_write;
  ...
  memory mem_i    (clk, cpu_addr, cpu_dout, mem_dout, write_mem);
...
module testbed();
  ...
  // instance of toy:
  stoy2 toy_i(clk, ext_din, ext_write, ext_dout);  

  initial
    begin
    `include "ulbel22_STOYcode.toy" //
    end
